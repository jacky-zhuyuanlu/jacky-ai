...
def get_llm_config(conf_path="system.conf"):
    config = configparser.ConfigParser()
    config.read(conf_path, encoding="utf-8")
    # 兼容无section的情况
    section = "key" if "key" in config else config.sections()[0]
    api_key = config.get(section, "gpt_api_key", fallback=None)
    base_url = config.get(section, "gpt_base_url", fallback=None)
    model_engine = config.get(section, "gpt_model_engine", fallback=None)
    return {
        "api_key": api_key,
        "base_url": base_url,
