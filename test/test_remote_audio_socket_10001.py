...
def receive_audio(client):
    ...
    while True:
        ...
        if b"\x00\x01\x02\x03\x04\x05\x06\x07\x08" == data: #文件开始传输标志
            ...
            is_speaking = False
        elif b"\x01\x02\x03\x04\x05\x06\x07\x08\x09" == data: #文本开始传输标志
            while True:
                data = client.recv(1024)
                filedata += data
                filedata = filedata.replace(b"\xf0\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8", b"") #去除心跳信息
                if b"\x09\x08\x07\x06\x05\x04\x03\x02\x01" == filedata[-9:]:#文本结束传输标志
                    filedata = filedata[:-9]
                    break
            # 解析文本内容
            text_content = filedata.decode('utf-8')
            print(f"\n接收到文本内容: {text_content}")
