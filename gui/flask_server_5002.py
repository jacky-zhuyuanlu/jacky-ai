...
system_conf.read(os.path.join(os.path.dirname(__file__), '../system.conf'), encoding='utf-8')

def get_system_config():
    conf = {}
    conf['gpt_model_engine'] = system_conf.get('key', 'gpt_model_engine', fallback='')
    conf['gpt_base_url'] = system_conf.get('key', 'gpt_base_url', fallback='')
    conf['tts_module'] = system_conf.get('key', 'tts_module', fallback='')
    conf['ASR_mode'] = system_conf.get('key', 'ASR_mode', fallback='')
    return conf
...
def api_get_Msg():
    try:
        ...
        while i >= 0:
            ...
            relist.append(
                dict(type=list[i][0], way=list[i][1], content=list[i][2], createtime=list[i][3], timetext=timetext,
                     username=list[i][5], id=list[i][6], is_adopted=list[i][7]))
            i -= 1
        if fay_booter.is_running():
            wsa_server.get_web_instance().add_cmd({"liveState": 1})
...
def home_get():
    ...
    try:
        ...
        dynamic_data = {}
    return render_template('new_index.html', system_conf=conf, dynamic_data=dynamic_data)
...
def to_greet():
    ...
    # 初始化面试会话
    session = interview_mgr.get_session(username, dynamic_data_path=os.path.join(os.path.dirname(__file__), '../dynamic_data.json'), name=username)
    # 获取第一个主问题
    ...
    fay_booter.feiFei.on_interact(interact)
    return jsonify({'status': 'success', 'msg': '已进行面试题打招呼', 'data': welcome_text}), 200
...
@__app.route('/api/get-transcript', methods=['get'])  # 修改为GET方法
def api_get_transcript():
    try:
        # 从数据库获取所有消息记录（不区分用户）
        contentdb = content_db.new_instance()
        # 获取所有消息，按时间升序排列，不限制用户ID
        msg_list = contentdb.get_list('all', 'asc', 1000, 0)  # 使用0表示获取所有用户消息

        # 转换为所需的消息数组格式
        transcript = []
        for msg in msg_list:
            # 映射角色：'user'对应用户，其他类型默认为assistant
            role = 'user' if msg[0] == 'user' else 'assistant'
            transcript.append({
                "role": role,
                "content": msg[2],  # 直接用原始内容
                "timestamp": msg[3]
            })
        ...
        return jsonify({'error': f'获取转录文本时出错: {e}'}), 500
...
def api_start_genagents():
    try:
        ...
        instruction_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'genagents',
                                        'instruction.json')
...
@__app.route("/api/start-interview", methods=["POST"])
def start_interview():
    user_id = request.json.get("user_id")
    interview = interview_mgr.get_session(user_id)
    question = interview.get_current_question()
    return jsonify({"question": question})

@__app.route("/api/interview-message", methods=["POST"])
def interview_message():
    user_id = request.json.get("user_id")
    user_input = request.json.get("message")
    interview = interview_mgr.get_session(user_id)
    reply = interview.next(user_input)
    return jsonify({"reply": reply})

@__app.route('/retell', methods=['GET'])
def retell_home():
    return render_template('retell_home.html')

@__app.route('/retell/login', methods=['GET', 'POST'])
def retell_login():
    if request.method == 'POST':
        username = request.form.get('username')
        if username:
            session['retell_user'] = username
            # 登录成功后直接跳转到 5002 端口首页
            return redirect('http://127.0.0.1:5002/')
    return render_template('retell_login.html')

@__app.route('/retell/dashboard', methods=['GET'])
def retell_dashboard():
    username = session.get('retell_user')
    if not username:
        return redirect(url_for('retell_login'))
    return render_template('retell_dashboard.html', username=username)
