...
class InterviewService:
    ...
    def get_current_question(self):import importlib
import json
import time
import os
import pyaudio
...
from core.interview_manager import get_interview
...
def to_greet():
    ...
    try:
        ...
        questions_text = "；".join([f"{i+1}、{q['question']}" for i, q in enumerate(questions) if 'question' in q])
        welcome_text = (
            f"你好，我是小云，柳州人，目前担任IT面试官。很高兴见到你，我将围绕你的项目经验进行本次面试。准备好了吗？我们开始吧。以下是本次面试题：{questions_text}"
        )
    except Exception as e:
...
def retell_login():
    if request.method == 'POST':
        ...
        if username:
            ...
            return redirect(url_for('retell_dashboard'))
...
def start():
    ...
    def next(self, user_input):
        ...
        if not self.in_followup:
            # 生成追问
            prompt = f"针对以下面试问题的回答，生成一个有针对性的追问：\n问题：{self.get_current_question()}\n回答：{user_input}\n追问："
            followup = call_llm(prompt)
